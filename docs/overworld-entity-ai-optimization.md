...
已完成第一阶段：BattleResolver 不再执行远距视野追逃，只负责接触交战与战斗结算；远距追逃由 `DailyDecisionProcessor` / `PerceptionIntentResolver` 统一解析，并由 `OverworldIntentApplier` 写入状态。后续若需要回放式调试，可再引入 `VisionEvent` 中间产物记录感知事件。
...
- PerceptionIntentResolver 的追击/逃跑阈值逻辑
