...
public sealed class LuaScriptEngine : IDisposable
{
	...
	private const string CareerSkillScriptPath = "res://scripts/career_skills/";
	...
	public (bool success, string? error) ValidateSkillScript(string skillId)
	{
		try
		{
			EnsureLibLoaded();
		...
		{
			string error = $"[Lua Load Error] {skillId}: {ex.Message}";
			GD.PushError(error);
			return (false, error);
		}
	}
	...
	public int PreloadAll()
	{
		...
		// 扫描职业技能目录
		int careerCount = PreloadDirectory(CareerSkillScriptPath);
		if (careerCount > 0)
			GD.Print($"[LuaScriptEngine] Loaded {careerCount} career skill scripts from {CareerSkillScriptPath}");
		...
		return count + careerCount + modCount;
	}
	...
	/// <summary>检查指定技能是否有对应的 Lua 脚本文件（内置、职业技能或 Mod）</summary>
	public bool HasScript(string skillId)
	{
		...
		// 检查职业技能
		string careerPath = CareerSkillScriptPath + skillId + ".lua";
		if (FileAccess.FileExists(careerPath)) return true;
		// 检查 Mod
	...
	private void EnsureSkillLoaded(string skillId)
	{
		...
		// 优先内置，然后职业技能，最后 Mod 目录
		string path = SkillScriptPath + skillId + ".lua";
		if (!FileAccess.FileExists(path))
		{
			path = CareerSkillScriptPath + skillId + ".lua";
			if (!FileAccess.FileExists(path))
			{
