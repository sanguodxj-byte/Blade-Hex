...
public partial class HexGrid : Node3D
{
    ...
    // 注意：在 C# 中通常不 preload .gd，而是动态创建类或加载 .tscn
    // 这里我们假设 HexCell 已迁移为 C# 类
    ...
    public List<Vector2I> FindPath(Vector2I startPos, Vector2I targetPos)
    {
        ...
        // 优先级队列简易实现
        var frontier = new SortedDictionary<int, List<Vector2I>>();
        
        AddToFrontier(frontier, startPos, 0.0f);
        ...
        while (frontier.Count > 0)
        {
            var current = PopFromFrontier(frontier);
            ...
            for (int dir = 0; dir < 6; dir++)
            {
                ...
                if (!costSoFar.ContainsKey(nextPos) || newCost < costSoFar[nextPos])
                {
                    ...
                    AddToFrontier(frontier, nextPos, priority);
    ...
    private void AddToFrontier(SortedDictionary<int, List<Vector2I>> frontier, Vector2I pos, float priority)
    {
        int bucket = (int)(priority * 10);
        if (!frontier.ContainsKey(bucket))
            frontier[bucket] = new List<Vector2I>();
        frontier[bucket].Add(pos);
    }

    private Vector2I PopFromFrontier(SortedDictionary<int, List<Vector2I>> frontier)
    {
        var first = frontier.First();
        var bucketArray = first.Value;
        var result = bucketArray[bucketArray.Count - 1];
        bucketArray.RemoveAt(bucketArray.Count - 1);

        if (bucketArray.Count == 0)
            frontier.Remove(first.Key);
