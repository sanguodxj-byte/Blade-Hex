...
// 迁移自 GDScript StatusEffectData.gd
using Godot;
...
public partial class StatusEffectData : Resource
{
    ...
    [Export] public bool IsNegative = true;
    [Export] public int DefaultDuration = 3;

    // 每回合伤害骰子
    [Export] public int TickDamageDiceCount;
    [Export] public int TickDamageDiceSides;
    [Export] public string TickDamageType = "";

    // 属性修正
    [Export] public Godot.Collections.Dictionary StatModifiers = new();

    // 可通过豁免解除
    [Export] public string SaveToRemove = "";
    [Export] public int SaveDc = 12;

    // 可解除的其他效果
    [Export] public string[] RemovesEffects = [];

    // 互斥标签
    [Export] public string CancelTag = "";
