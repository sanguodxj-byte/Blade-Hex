...
// 所有生物统一：1级属性25，每级+1
// 对应策划05-角色与职业.md
// 迁移自 GDScript CharacterGenerator.gd (781 lines)
using Godot;
using System;
...
public static class CharacterGenerator
{
    ...
    public static UnitData GenerateCharacter(RaceData? race = null, int level = 1, long seedVal = -1)
    {
        ...
        unitData.UnitName = GenerateName(race);
    ...
    static Dictionary<string, float> GetAllocationWeights(RaceData? race)
    {
        ...
        if (race.StrMod > 0) w["str"] += race.StrMod * 0.8f;
        if (race.DexMod > 0) w["dex"] += race.DexMod * 0.8f;
        if (race.ConMod > 0) w["con"] += race.ConMod * 0.8f;
        if (race.IntMod > 0) w["intel"] += race.IntMod * 0.8f;
        if (race.WisMod > 0) w["wis"] += race.WisMod * 0.8f;
        if (race.ChaMod > 0) w["cha"] += race.ChaMod * 0.8f;
        return w;
    }
    ...
    static void ApplyFunctionalTraits(UnitData u, TraitData[] traits)
    ...
    public static void AiAutoLevel(UnitData u, int targetLevel)
    {
        ...
        u.Level = targetLevel; u.SkillPoints = targetLevel - 1;
    ...
    static string GenerateName(RaceData race)
    {
        ...
        string rk = race.raceId switch
        { RaceData.Race.Human=>"human", RaceData.Race.Elf=>"elf", RaceData.Race.Dwarf=>"dwarf",
          RaceData.Race.HalfOrc=>"half_orc", RaceData.Race.HalfElf=>"half_elf", _=>"human" };
        var n = names.GetValueOrDefault(rk, names["human"]);
        return n[GD.Randi() % (uint)n.Length];
    }
    ...
    static SpellData? CreateSpellById(string spellId, int level)
    {
        ...
        switch (spellId)
        {
            ...
            case "entangle":
                ...
                break;
            case "holy_light":
                spell.SpellName = "holy_light"; spell.spellSchool = SpellData.SpellSchool.Abjuration;
            ...
            case "smite":
                ...
                spell.resolutionType = SpellData.ResolutionType.AttackRoll;
                spell.DamageDiceCount = 3 + Mathf.FloorToInt(level / 4.0f); spell.DamageDiceSides = 8; spell.DamageType = "radiant";
                break;
            case "healing_light":
    ...
    {
        "盾墙" or "坚守阵地" or "影遁" or "魔法护盾" or "神圣护盾" => 0,
