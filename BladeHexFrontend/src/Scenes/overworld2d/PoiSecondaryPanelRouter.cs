...
internal sealed class PoiSecondaryPanelRouter
{
    ...
    private readonly Func<TournamentService?> _tournamentService;
    private readonly Func<OverworldEntityManager?> _entityMgr;
    private readonly Func<ReputationTracker?> _reputationTracker;
    private readonly Action<BattleContext, int> _arenaCombatRequested;
    private readonly Action<BattleContext, Godot.Collections.Dictionary> _tournamentCombatRequested;
    ...
    public PoiSecondaryPanelRouter(
        ...
        Func<TournamentService?> tournamentService,
        Func<OverworldEntityManager?> entityMgr,
        Func<ReputationTracker?> reputationTracker,
        Action<BattleContext, int> arenaCombatRequested,
        Action<BattleContext, Godot.Collections.Dictionary> tournamentCombatRequested,
        Action cleanupInteraction)
    {
        ...
        _tournamentCombatRequested = tournamentCombatRequested;
    ...
    {
        GD.Print($"[PoiSecondaryPanelRouter] Open called: {facilityType}");
        GD.Print($"[PoiSecondaryPanelRouter] _townPanel={_townPanel() != null}, _currentTown={_currentTown() != null}");
        GD.Print($"[PoiSecondaryPanelRouter] _economy={_economy() != null}, _playerParty={_playerParty() != null}");
    ...
    private void OpenTradePanel()
    {
        ...
        IOverworldContext? scene = _panelParent as IOverworldContext;
        var entityMgr = _entityMgr();

        if (entityMgr != null)
        {
            poi = entityMgr.Pois.FirstOrDefault(p => p.PoiName == shopName);
            reputation = _reputationTracker();
            worldEngine = entityMgr.WorldEngine;
        }

        ui.OpenPartyShop(shopName, economy, MarketStockService.GenerateStock(prosperity), prosperity, poi, reputation, worldEngine, scene);
    }
    ...
    private void OpenRecruitPanel()
    {
        ...
        _recruitPanel.ShowRecruitList(CreatePanelContext(), instantOverlay: true);
    }

    private void OpenSmithyPanel()
    {
        GD.Print("[PoiSecondaryPanelRouter] OpenSmithyPanel called");
        _smithyPanel ??= AddPanel(new SmithyPanel(), panel =>
        {
            panel.SmithyFinished += ReturnToTownPanel;
        });

        var ctx = CreatePanelContext();
        GD.Print($"[PoiSecondaryPanelRouter] Context: Economy={ctx.Economy != null}, Roster={ctx.Roster != null}");
        _smithyPanel.ShowSmithy(ctx, instantOverlay: true);
    }
    ...
    private void OpenTemplePanel()
    {
        ...
        _templePanel.ShowTemple(CreatePanelContext(), instantOverlay: true);
    }
    ...
    private void OpenArenaPanel()
    {
        _arenaPanel ??= AddPanel(new ArenaPanel(), panel =>
        {
            ...
            panel.TournamentCombatRequested += OnTournamentCombatRequested;
        });

        var economy = _economy();
        var tournamentService = _tournamentService();
        var town = _currentTown();
        var party = _playerParty();
        
        int playerLevel = party?.Roster?.Leader?.Level ?? 1;
        string hostTownName = town?.TownName ?? "竞技场";
        string hostNationId = town?.Faction ?? "";
        
        if (economy != null) _arenaPanel.ShowArena(economy, tournamentService, playerLevel, hostTownName, hostNationId, instantOverlay: true);
        else _arenaPanel.ShowPanel(instantOverlay: true);
    }
    ...
    {
        _arenaPanel?.HidePanel();
        _tournamentCombatRequested(ctx, state);
    }
    ...
    private void OpenQuestPanel()
    {
        ...
        _questBoardPanel.ShowBoardDynamic(CreatePanelContext(), instantOverlay: true);
    }
    ...
    private void OpenRestPanel()
    {
        ...
        _restPanel.ShowRest(CreatePanelContext(), instantOverlay: true);
    }
    ...
    private void ReturnToTownPanel()
    {
        ...
        if (town != null && GodotObject.IsInstanceValid(town) && panel != null)
            panel.ShowTown(town, instantOverlay: true);
        else
            _cleanupInteraction();
    }
}
