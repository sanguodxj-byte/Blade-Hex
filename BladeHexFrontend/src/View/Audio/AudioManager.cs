...
/// <summary>
/// 全局音频管理器 (Autoload Singleton)。
/// 管理 BGM 交叉淡入淡出、SFX 池化播放、Ambient 循环播放和地形脚步声映射。
/// </summary>
[GlobalClass]
public partial class AudioManager : Node
{
    ...
    private void InitBgmPlaylists()
    {
        ...
        // 大地图旅行（昼）
        AddBgmVariant(Scenario.Overworld, "default", BgmBasePath + "overworld_travel.ogg");

        // 大地图夜晚（3个变体随机）— Gemini 确认这3首都是慢节奏忧郁氛围
        AddBgmVariant(Scenario.Overworld, "night", BgmBasePath + "overworld_night.ogg");
        AddBgmVariant(Scenario.Overworld, "night", BgmBasePath + "overworld_night_2.ogg");
        AddBgmVariant(Scenario.Overworld, "night", BgmBasePath + "overworld_night_3.ogg");

        // 大地图危险/雷暴
        AddBgmVariant(Scenario.Overworld, "danger", BgmBasePath + "overworld_storm.ogg");

        // 战斗场景 — 当前只有1首真正快节奏战斗BGM
        ...
        AddBgmVariant(Scenario.Dungeon, "default", BgmBasePath + "prox_ruins_3.ogg");

        // 剧情事件 — 戏剧性 + 平静两种
        AddBgmVariant(Scenario.Event, "default", BgmBasePath + "event_dramatic.ogg");
        AddBgmVariant(Scenario.Event, "calm", BgmBasePath + "event_calm.ogg");
        AddBgmVariant(Scenario.Event, "dramatic", BgmBasePath + "event_dramatic.ogg");
    ...
    {
        // 仅 ui_click.mp3 实际存在，其他 UI 音效都退化到 ui_click
        string click = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("ui_click", click);
        RegisterSfx("ui_hover", click);
        RegisterSfx("ui_error", click);
        RegisterSfx("ui_panel_open", click);
        RegisterSfx("ui_panel_close", click);
        RegisterSfx("ui_tab_switch", click);
        RegisterSfx("ui_checkbox", click);
        RegisterSfx("ui_slider", click);
        RegisterSfx("ui_tooltip_show", click);
        RegisterSfx("ui_gold_change", click);
        RegisterSfx("ui_level_up", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("ui_notification", click);
        RegisterSfx("ui_quest_accept", click);
        RegisterSfx("ui_quest_complete", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("ui_save", click);
        RegisterSfx("ui_load", click);
    }
    ...
    private void InitCombatAttackSfx()
    {
        ...
        // combat_fumble 资源缺失 — 退化到 sword_miss
        RegisterSfx("combat_fumble", SfxBasePath + "combat/attack/sword_miss_1.wav");
    ...
    private void InitCombatSkillSfx()
    {
        ...
        // aimed_shot/scatter_shot 资源缺失 — 退化到 double_shot
        RegisterSfx("skill_aimed_shot", SfxBasePath + "combat/skill/double_shot.mp3");
        RegisterSfx("skill_scatter_shot", SfxBasePath + "combat/skill/double_shot.mp3");

        // 魔法系 — 缺失资源退化到 holy_judgment / nature_wrath
        RegisterSfx("skill_mana_shield", SfxBasePath + "combat/skill/holy_shield.mp3");
        ...
        RegisterSfx("skill_arcane_shield", SfxBasePath + "combat/skill/holy_shield.mp3");
        ...
        RegisterSfx("skill_shadow_clone", SfxBasePath + "combat/skill/stealth.mp3");
        RegisterSfx("skill_taunt", SfxBasePath + "combat/skill/war_cry_1.mp3");
        RegisterSfx("skill_bulwark", SfxBasePath + "combat/skill/shield_bash_1.mp3");
        RegisterSfx("skill_rally", SfxBasePath + "combat/skill/war_cry_2.mp3");
        RegisterSfx("skill_intimidate", SfxBasePath + "combat/skill/war_cry_1.mp3");
        RegisterSfx("skill_heroic_call", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("skill_inspire", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("skill_dispel", SfxBasePath + "combat/skill/holy_shield.mp3");
    }

    private void InitCombatSpellSfx()
    {
        // 法术学派音效 — 资源完全缺失，全部退化到通用技能音效避免运行时报错
        // 火/冰/雷/土 → nature_wrath; 圣/暗/奥/自然 → holy_judgment
        string[] elementalSchools = { "fire", "ice", "lightning", "earth" };
        string[] arcaneSchools = { "holy", "shadow", "arcane", "nature" };
        foreach (string school in elementalSchools)
        {
            RegisterSfx($"spell_{school}_cast", SfxBasePath + "combat/skill/nature_wrath.mp3");
            RegisterSfx($"spell_{school}_impact", SfxBasePath + "combat/skill/nature_wrath.mp3");
        }
        foreach (string school in arcaneSchools)
        {
            RegisterSfx($"spell_{school}_cast", SfxBasePath + "combat/skill/holy_judgment.mp3");
            RegisterSfx($"spell_{school}_impact", SfxBasePath + "combat/skill/holy_judgment.mp3");
        }

        RegisterSfx("spell_no_mana", SfxBasePath + "ui/ui_click.mp3");
        RegisterSfx("spell_cooldown", SfxBasePath + "ui/ui_click.mp3");
    }

    private void InitCombatStatusSfx()
    {
        // 状态音效资源缺失 — 退化到 ui_click，保证不报错
        string fallback = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("status_burning", fallback);
        RegisterSfx("status_freezing", fallback);
        RegisterSfx("status_poison", fallback);
        RegisterSfx("status_bleed", fallback);
        RegisterSfx("status_stun", fallback);
        RegisterSfx("status_root", fallback);
        RegisterSfx("status_cure", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("status_rally", SfxBasePath + "combat/skill/war_cry_1.mp3");
        RegisterSfx("status_rout", SfxBasePath + "combat/skill/war_cry_2.mp3");
    }

    private void InitCombatFlowSfx()
    {
        // 流程音效资源缺失 — 退化到 ui_click 或战斗音效
        string fallback = SfxBasePath + "ui/ui_click.mp3";

        // 回合流程
        RegisterSfx("combat_turn_start", fallback);
        RegisterSfx("combat_enemy_turn", fallback);
        RegisterSfx("combat_victory", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("combat_defeat", SfxBasePath + "combat/attack/death_1.mp3");

        // 战斗事件
        RegisterSfx("combat_counter", SfxBasePath + "combat/attack/sword_crit_1.mp3");
        RegisterSfx("combat_aoo", SfxBasePath + "combat/attack/sword_hit_1.mp3");
        RegisterSfx("combat_flanking", SfxBasePath + "combat/skill/poison_blade.mp3");
        RegisterSfx("combat_charge", SfxBasePath + "combat/skill/whirlwind.mp3");
        RegisterSfx("combat_mount_charge", SfxBasePath + "combat/skill/whirlwind.mp3");

        // 环境事件
        RegisterSfx("combat_env_storm", fallback);
        RegisterSfx("combat_env_fog", fallback);
        RegisterSfx("combat_env_quake", fallback);
        RegisterSfx("combat_env_poison_fog", fallback);
        RegisterSfx("combat_env_lava", fallback);
    }

    private void InitMovementSfx()
    {
        // 资源缺失 — 全部退化到 ui_click，避免运行时报错
        string fallback = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("move_footstep_grass", fallback);
        RegisterSfx("move_footstep_stone", fallback);
        RegisterSfx("move_footstep_snow", fallback);
        RegisterSfx("move_footstep_mud", fallback);
        RegisterSfx("move_footstep_wood", fallback);
        RegisterSfx("move_footstep_water", fallback);
        RegisterSfx("move_footstep_sand", fallback);

        // 单位选择与路径
        RegisterSfx("move_unit_select", fallback);
        RegisterSfx("move_unit_deselect", fallback);
        RegisterSfx("move_cell_highlight", fallback);
        RegisterSfx("move_path_confirm", fallback);
        RegisterSfx("move_weapon_switch", fallback);
    }

    private void InitOverworldSfx()
    {
        // 资源缺失 — 退化到 ui_click 或最相关的现有音效
        string fallback = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("ow_travel_start", fallback);
        RegisterSfx("ow_travel_stop", fallback);
        RegisterSfx("ow_encounter", SfxBasePath + "combat/skill/war_cry_1.mp3");
        RegisterSfx("ow_town_enter", fallback);
        RegisterSfx("ow_town_leave", fallback);
        RegisterSfx("ow_combat_trigger", SfxBasePath + "combat/skill/war_cry_1.mp3");
        RegisterSfx("ow_fog_reveal", fallback);
        RegisterSfx("ow_day_cycle", fallback);
        RegisterSfx("ow_season_change", fallback);
        RegisterSfx("ow_enemy_sighted", SfxBasePath + "combat/skill/war_cry_2.mp3");
        RegisterSfx("ow_poi_discover", SfxBasePath + "combat/skill/blessing.mp3");
    }
    ...
    private void InitTownSfx()
    {
        // 资源缺失 — 退化到 ui_click
        string fallback = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("town_trade_buy", fallback);
        RegisterSfx("town_trade_sell", fallback);
        RegisterSfx("town_trade_fail", fallback);
        RegisterSfx("town_rest_inn", fallback);
        RegisterSfx("town_temple_heal", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("town_smithy_upgrade", SfxBasePath + "combat/attack/armor_break.mp3");
        RegisterSfx("town_smithy_fail", fallback);
        RegisterSfx("town_recruit", fallback);
        RegisterSfx("town_train", fallback);
        RegisterSfx("town_arena_fight", SfxBasePath + "combat/skill/war_cry_1.mp3");
        RegisterSfx("town_arena_win", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("town_repair", fallback);
    }

    private void InitCharacterSfx()
    {
        string fallback = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("char_node_activate", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("char_node_locked", fallback);
        RegisterSfx("char_spell_learn", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("char_equip_change", fallback);
        RegisterSfx("char_equip_fail", fallback);
        RegisterSfx("char_stat_increase", fallback);
    }

    private void InitQuestSfx()
    {
        string fallback = SfxBasePath + "ui/ui_click.mp3";
        RegisterSfx("quest_new", fallback);
        RegisterSfx("quest_accept", fallback);
        RegisterSfx("quest_progress", fallback);
        RegisterSfx("quest_complete", SfxBasePath + "combat/skill/blessing.mp3");
        RegisterSfx("quest_fail", fallback);
        RegisterSfx("quest_expire", fallback);
    }
    ...
    /// <summary>检查是否注册了指定名称的音效</summary>
