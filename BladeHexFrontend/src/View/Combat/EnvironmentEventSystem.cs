...
{
    public enum EnvironmentEventType { Storm, Fog, Earthquake, PoisonFog, ArcaneFlare, Lava }
    ...
    private static readonly Dictionary<EnvironmentEventType, int> EventIntervals = new()
    {
        ...
        { EnvironmentEventType.ArcaneFlare, 1 },
        ...
        foreach (var eventType in ActiveEvents)
        {
            ...
            switch (eventType)
            {
                ...
                case EnvironmentEventType.PoisonFog:
                    ...
                    break;
                case EnvironmentEventType.ArcaneFlare:
                    ProcessArcaneFlare(allUnits);
                    break;
    ...
    {
        int destroyCount = GD.RandRange(2, 4);
        var cells = grid.GetCells().ToList();
        if (cells.Count == 0) return;

        for (int i = 0; i < destroyCount; i++)
        {
            var cell = cells[GD.RandRange(0, cells.Count - 1)];
            // 将建筑废墟变为平地，墙壁变为废墟
            if (cell.Data != null)
            {
    ...
    private void ProcessArcaneFlare(IEnumerable<Unit> allUnits)
    {
